 ...
 \par  {\bf i. M/M/1 Queue model*}
  \par we can rewrite the \ref{eq1} as
				...
				\begin{equation}    
					\left  \{
       						\begin{array}{lr}
			 				L_q = (1-\xi)\frac {\rho ^2}{1-\rho}\\ % \label{eq4.34}
			 ...
			 In the same way,we can rewrite the \ref{eq2} as
				\begin{equation}    
					\left  \{
       						\begin{array}{lr}
       							W_s = (1-\xi)\frac{1}{\mu - \lambda}\\  % \label{eq4.35}
...
\end{equation}
	...
	\end{enumerate}
	%\par Then,we take the two modifications into consideration to analyze its feasibility.
	
	%Then, we take into consideration the second modification. Define 
